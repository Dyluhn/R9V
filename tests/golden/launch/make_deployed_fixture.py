...
"""Normalize a GPU-tested container's Docker create payload into the launch-parity fixture.

Usage: make_deployed_fixture.py CREATE_JSON OVERLAY_DIR PLACEMENT_JSON IMAGE_ENV_JSON SOURCE > fixture.json

CREATE_JSON is the create payload of the container that was GPU-tested,
OVERLAY_DIR the directory it mounted at /r9v-full-mutable (every file in it is
pinned by SHA-256), PLACEMENT_JSON the file it mounted at /placement/experts.json,
IMAGE_ENV_JSON the output of `docker image inspect IMAGE --format
'{{json .Config.Env}}'`, and SOURCE one line naming where CREATE_JSON came from.
Host-specific values are replaced by bind target and environment key, so the
fixture holds no host paths: bind sources become tokens, and the cache
namespace, container name and GPU BDFs become fixed placeholders the test
passes to the launcher.
"""
...
def main():
    create_path, overlay_dir, placement_path, image_env_path = map(Path, sys.argv[1:5])
    source = sys.argv[5]
    ...
    fixture = {
        ...
        "image_env": image_env,
        "overlay_sha256": {path.name: hashlib.sha256(path.read_bytes()).hexdigest()
