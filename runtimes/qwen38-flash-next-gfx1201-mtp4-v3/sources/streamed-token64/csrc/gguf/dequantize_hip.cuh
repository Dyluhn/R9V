...
template <int qk, int qr, dequantize_kernel_t dequantize_kernel, typename dst_t>
static void dequantize_block_cuda(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k, hipStream_t stream) {
    const int64_t num_blocks = (k + 2*CUDA_DEQUANTIZE_BLOCK_SIZE - 1) / (2*CUDA_DEQUANTIZE_BLOCK_SIZE);
   hipLaunchKernelGGL(( dequantize_block<qk, qr, dequantize_kernel>), dim3(num_blocks), dim3(CUDA_DEQUANTIZE_BLOCK_SIZE), 0, stream, vx, y, k);
}
...
template<typename dst_t>
static void dequantize_row_iq2_xxs_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq2_xxs), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq2_xs_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq2_xs), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq2_s_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq2_s), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq3_xxs_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq3_xxs), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq3_s_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq3_s), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq1_s_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq1_s), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq1_m_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = k / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq1_m), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq4_nl_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = (k + QK_K - 1) / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq4_nl), dim3(nb), dim3(32), 0, stream, vx, y);
}

template<typename dst_t>
static void dequantize_row_iq4_xs_cuda(const void * vx, dst_t * y, const int64_t k, hipStream_t stream) {
    const int nb = (k + QK_K - 1) / QK_K;
   hipLaunchKernelGGL(( dequantize_block_iq4_xs), dim3(nb), dim3(32), 0, stream, vx, y);
}
