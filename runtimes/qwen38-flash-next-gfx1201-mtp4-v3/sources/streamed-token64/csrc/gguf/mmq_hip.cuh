...
static void ggml_mul_mat_q4_0_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
       ...
       hipLaunchKernelGGL(( mul_mat_q4_0<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
...
static void ggml_mul_mat_q4_1_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q4_1<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q4_1<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q5_0_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q5_0<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q5_0<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q5_1_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q5_1<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q5_1<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q8_0_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q8_0<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q8_0<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q2_K_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q2_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q2_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q3_K_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q3_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q3_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q4_K_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q4_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q4_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q5_K_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q5_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q5_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
...
static void ggml_mul_mat_q6_K_q8_1_cuda(
    ...
    if (nrows_x % mmq_y == 0) {
        const bool need_check = false;
       hipLaunchKernelGGL(( mul_mat_q6_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
            vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
    } else {
        const bool need_check = true;
       hipLaunchKernelGGL(( mul_mat_q6_K<scalar_t, need_check>), dim3(block_nums), dim3(block_dims), 0, stream, 
