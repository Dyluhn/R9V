...
constexpr int kTokensPerTile = 16; // 16 tokens per WMMA fragment
...
// ============================================================================
// 3. Dedicated Streamed Dense Q8_0 x Q8_1 WMMA Kernel
// Parameterized by kNumTokenTiles:
//   kNumTokenTiles = 2: 32 tokens/workgroup (exact selected token32 control)
//   kNumTokenTiles = 4: 64 tokens/workgroup (token64 candidate)
//
// Bounded LDS tile (8 blocks = 256 K elements per chunk).
// Reuses each staged 8-block weight tile across kNumTokenTiles 16-token fragments.
...
void dense_q8_0_q8_1_wmma_streamed_kernel(
  ...
  constexpr int kTokensPerBlock = kNumTokenTiles * kTokensPerTile;
  ...
  // Setup activation pointers for this lane's tokens (tiles n=0..kNumTokenTiles-1)
  const int* act[kNumTokenTiles];
  int token_ids[kNumTokenTiles];
  const int col0 = blockIdx.y * kTokensPerBlock;
#pragma unroll
  for (int n = 0; n < kNumTokenTiles; ++n) {
    const int t = col0 + n * kTokensPerTile + idx;
    token_ids[n] = t;
  ...
  // Persistent accumulator registers for kNumTokenTiles token tiles x 8 rows across all K chunks
  float acc[kNumTokenTiles][8];
#pragma unroll
  for (int n = 0; n < kNumTokenTiles; ++n) {
#pragma unroll
    for (int i = 0; i < 8; ++i) {
...
// Variants:
//   0: Control reference MMQ
//   1: Selected Control Streamed WMMA Token32 (2 tiles/block)
//   2: Candidate Streamed WMMA Token64 (4 tiles/block)
// ============================================================================
...
torch::Tensor q8_matmul(
    ...
    int variant = 2; // Default to candidate token64
    if (py::isinstance<py::int_>(variant_obj)) {
        ...
        if (s == "candidate" || s == "token64" || s == "streamed64") variant = 2;
        else if (s == "control" || s == "token32" || s == "streamed32") variant = 1;
        else if (s == "mmq" || s == "oracle") variant = 0;
        else TORCH_CHECK(false, "Unknown variant string: ", s);
    } else {
        TORCH_CHECK(false, "variant must be int (0/1/2) or string ('control'/'candidate'/'token32'/'token64'/'mmq')");
    }

    TORCH_CHECK(variant >= 0 && variant <= 2,
        "Invalid variant: expected 0 (MMQ), 1 (token32 control), or 2 (token64 candidate), got ", variant);
    ...
    if (variant == 0) {
        // Oracle: Reference source MMQ path with guarded allocation
        ...
        // Control: Dedicated Streamed gfx12 WMMA Kernel with 32 tokens/workgroup
        const int block_num_x = static_cast<int>((N + kRowsPerBlock - 1) / kRowsPerBlock);
        const int block_num_y = static_cast<int>((M + 32 - 1) / 32);
        const dim3 block_nums(block_num_x, block_num_y, 1);
        const dim3 block_dims(kThreads, 1, 1);

        hipLaunchKernelGGL(
            (dense_q8_0_q8_1_wmma_streamed_kernel<2>),
            block_nums,
            block_dims,
            0,
            stream,
        ...
        // Candidate: Dedicated Streamed gfx12 WMMA Kernel with 64 tokens/workgroup
        const int block_num_x = static_cast<int>((N + kRowsPerBlock - 1) / kRowsPerBlock);
        const int block_num_y = static_cast<int>((M + 64 - 1) / 64);
        const dim3 block_nums(block_num_x, block_num_y, 1);
        const dim3 block_dims(kThreads, 1, 1);

        hipLaunchKernelGGL(
            (dense_q8_0_q8_1_wmma_streamed_kernel<4>),
            block_nums,
            block_dims,
            0,
            stream,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.doc() = "Prefill Q8_0 Streamed WMMA Token64 Extension (Variant 0: MMQ Oracle, Variant 1: Token32 Control, Variant 2: Token64 Candidate)";
    m.def("q8_matmul", &prefill_q8_wmma_streamed::q8_matmul,
          "Execute Q8_0 matmul: Y = X @ W^T with selectable variant (0: MMQ, 1: Token32 Control, 2: Token64 Candidate)",
          py::arg("W"), py::arg("X"), py::arg("variant") = py::int_(2));
    m.def("q8_matmul_control", [](torch::Tensor W, torch::Tensor X) {
        return prefill_q8_wmma_streamed::q8_matmul(W, X, py::int_(1));
    }, "Execute Q8_0 matmul with selected Token32 Control WMMA", py::arg("W"), py::arg("X"));
    m.def("q8_matmul_candidate", [](torch::Tensor W, torch::Tensor X) {
        return prefill_q8_wmma_streamed::q8_matmul(W, X, py::int_(2));
    }, "Execute Q8_0 matmul with Candidate Token64 WMMA", py::arg("W"), py::arg("X"));
    m.def("q8_matmul_token32", [](torch::Tensor W, torch::Tensor X) {
        return prefill_q8_wmma_streamed::q8_matmul(W, X, py::int_(1));
    }, "Execute Q8_0 matmul with Token32 WMMA", py::arg("W"), py::arg("X"));
    m.def("q8_matmul_token64", [](torch::Tensor W, torch::Tensor X) {
        return prefill_q8_wmma_streamed::q8_matmul(W, X, py::int_(2));
    }, "Execute Q8_0 matmul with Token64 WMMA", py::arg("W"), py::arg("X"));
    m.def("q8_matmul_mmq", [](torch::Tensor W, torch::Tensor X) {
        return prefill_q8_wmma_streamed::q8_matmul(W, X, py::int_(0));
    }, "Execute Q8_0 matmul with reference MMQ oracle", py::arg("W"), py::arg("X"));
}
