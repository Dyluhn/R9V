 ...
 * Parameterized for Token32 control (2 tiles/block) and Token64 candidate (4 tiles/block).
 * Tests exact fragment layout, chunk streaming, scale ordering, M boundary masks, and zero-overread memory access.
 */
...
constexpr int K_TOKENS_PER_TILE = 16;
constexpr int K_TOKENS_PER_BLOCK_TOKEN32 = 32; // 2 tiles
constexpr int K_TOKENS_PER_BLOCK_TOKEN64 = 64; // 4 tiles
...
// Token mapping simulation for token32 (kNumTiles=2) and token64 (kNumTiles=4)
template <int kNumTokenTiles>
struct TokenMappingSim {
    static constexpr int kTokensPerBlock = kNumTokenTiles * K_TOKENS_PER_TILE;

    static int get_token_id(int block_idx_y, int tile_n, int lane) {
        int col0 = block_idx_y * kTokensPerBlock;
        int idx = lane & 15;
        return col0 + tile_n * K_TOKENS_PER_TILE + idx;
    }

    static bool is_token_active(int token_id, int M) {
        return token_id >= 0 && token_id < M;
    }

    static int get_output_row(int block_idx_x, int wave, int lane, int reg_i) {
        int row0 = block_idx_x * K_ROWS_PER_BLOCK + wave * K_ROWS_PER_WAVE;
        int half = lane >> 4;
        return row0 + 8 * half + reg_i;
    }
};
