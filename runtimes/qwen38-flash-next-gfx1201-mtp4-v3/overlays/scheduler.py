...
# R9V CED (approximate late-layer prefill). On only when R9V_CED_PROJECTOR names the projector
# file (its keys declare the split). The workers run layers 0..split-1 of an approximate chunk
# and fill the later layers' caches from the projector; the exact tail and decode run the full
# model. Service settings (container env, validated at startup):
#   R9V_CED_DEFAULT     on | off: CED for requests that do not set r9v_ced (default on)
#   R9V_CED_MIN_PROMPT  prompts shorter than this stay exact under the default (default 8192)
#   R9V_CED_TAIL        exact tail in tokens, rounded up to a cache block (default 2048, >= 512)
# Per request (vllm_xargs): r9v_ced true/false forces CED on (any length) or off;
# r9v_ced_tail sets the exact tail (raised to 512 if lower). Prompts with images or video stay
# exact, and so do requests asking for prompt logprobs unless they set r9v_ced (approximated
# positions have no meaningful logits). CED requests get their own prefix-cache namespace
# (salt with projector, split and tail), so no exact request ever reuses their KV blocks or
# GDN states. Research only: R9V_CED_CONTROL names a file read at each new request, "off" or
# TAIL[@SPLIT][/full], which overrides the policy; with it, R9V_CED_PROJECTOR may list one
# projector per split.
CED_MIN_TAIL = 512  # the shortest exact tail the grade scored


@dataclass(frozen=True)
class CedPlan:
    tail_start: int  # first prompt position computed exactly (block-aligned)
    split: int  # first approximated layer
    fill: str  # "lean" cache writes, or "full" attention blocks (research timing A/B)
    salt: str  # prefix-cache namespace of this plan
...
@dataclass(frozen=True)
class CedPolicy:
    projectors: dict[int, str]  # split -> projector file name
    tail: int
    min_prompt: int
    default_on: bool
    control: str | None  # research control file

    @classmethod
    def from_env(cls) -> "CedPolicy | None":
        paths = [p for p in os.environ.get("R9V_CED_PROJECTOR", "").split(",") if p]
        if not paths:
            return None
        control = os.environ.get("R9V_CED_CONTROL")
        if len(paths) > 1 and not control:
            raise ValueError(f"R9V_CED_PROJECTOR lists several projectors, which is research-only (R9V_CED_CONTROL): {paths}")
        projectors = {_ced_split_of(p): os.path.basename(p) for p in paths}
        if len(projectors) != len(paths):
            raise ValueError(f"R9V_CED_PROJECTOR lists two projectors with the same split: {paths}")
        default = os.environ.get("R9V_CED_DEFAULT", "on")
        if default not in ("on", "off"):
            raise ValueError(f"R9V_CED_DEFAULT must be on or off, got {default!r}")
        policy = cls(projectors, _env_int("R9V_CED_TAIL", 2048, CED_MIN_TAIL),
                     _env_int("R9V_CED_MIN_PROMPT", 8192, 0), default == "on", control)
        logger.info("CED on: projectors %s, exact tail %d, min prompt %d, default %s%s", projectors, policy.tail,
                    policy.min_prompt, default, f", research control {control}" if control else "")
        return policy


def _ced_split_of(path: str) -> int:
    """The split a projector file declares: its first late layer ("layer.S".."layer.N" contiguous,
    plus "final")."""
    ...
    if not layers or "final" not in keys or layers != list(range(layers[0], layers[-1] + 1)):
        raise ValueError(f"CED projector {path}: expected keys layer.S..layer.N plus final")
    return layers[0]


@dataclass(frozen=True)
class CedArgs:
    """What a request asks for: r9v_ced (None = the service default), r9v_ced_tail, prompt logprobs.
    Invalid values are logged and ignored: a request must not be able to stop the engine."""
    on: bool | None = None
    tail: int | None = None
    prompt_logprobs: bool = False

    @classmethod
    def of(cls, params) -> "CedArgs":
        extra = (params.extra_args if params else None) or {}
        on_arg, tail_arg = extra.get("r9v_ced"), extra.get("r9v_ced_tail")
        on = _ON_VALUES.get(str(on_arg).strip().lower())  # vllm_xargs carries JSON true as 1
        if on_arg is not None and on is None:
            logger.warning("ignoring r9v_ced=%r (expected true or false)", on_arg)
        tail = None
        if tail_arg is not None:
            try:
                tail = max(int(str(tail_arg)), CED_MIN_TAIL)
            except ValueError:
                logger.warning("ignoring r9v_ced_tail=%r (expected an integer)", tail_arg)
        return cls(on, tail, params is not None and params.prompt_logprobs is not None)


_ON_VALUES = {"1": True, "true": True, "on": True, "0": False, "false": False, "off": False}


def ced_plan(policy: CedPolicy, num_prompt_tokens: int, block_size: int, args: CedArgs,
             control: str = "") -> CedPlan | None:
    """This prompt's CED plan, or None to prefill it exactly. control: the research control
    file's text ("" = no override)."""
    on, tail, split, fill = args.on, args.tail or policy.tail, None, "lean"
    if control == "off":
        return None
    if control:
        mode, _, fill = control.partition("/")
        tail_text, _, split_text = mode.partition("@")
        tail, split, fill = int(tail_text), int(split_text) if split_text else None, fill or "lean"
        if tail <= 0 or fill not in ("lean", "full"):
            raise ValueError(f"CED control {control!r}: expected off or TAIL[@SPLIT][/full]")
        on = True if on is None else on
    if on is None:
        on = policy.default_on and num_prompt_tokens >= policy.min_prompt and not args.prompt_logprobs
    tail_start = (num_prompt_tokens - tail) // block_size * block_size
    if not on or tail_start <= 0:
        return None
    if split is None:
        if len(policy.projectors) != 1:
            raise ValueError(f"CED: several projectors {sorted(policy.projectors)}; the control must name a split")
        split = next(iter(policy.projectors))
    if split not in policy.projectors:
        raise ValueError(f"CED control asks for split {split}; loaded projectors: {sorted(policy.projectors)}")
    return CedPlan(tail_start, split, fill, f"r9v-ced:{policy.projectors[split]}:split={split}:tail={tail}")
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.ced_policy = CedPolicy.from_env()
    ...
    def _ced_plan(self, request: Request) -> CedPlan | None:
        # Not for streamed input (prompt length not final) or images/video (the projector is fit on text).
        if self.ced_policy is None or request.resumable or request.mm_features:
            return None
        control = ""
        if self.ced_policy.control:
            try:
                with open(self.ced_policy.control) as f:
                    control = f.read().strip()
            except FileNotFoundError:
                pass
        return ced_plan(self.ced_policy, request.num_prompt_tokens, self.block_size,
                        CedArgs.of(request.sampling_params), control)
    ...
    def _mamba_block_aligned_split(
        ...
        stops = (
            ...
            # CED: approximate chunks end exactly where the exact tail begins.
            request.r9v_ced.tail_start if request.r9v_ced else 0,
        )
    ...
    def schedule(self, throttle_prefills: bool = False) -> SchedulerOutput:
        ...
        # CED: tag the step approximate when its only request is prefilling entirely
        # before its exact tail. Workers read these (plain attributes, pickled).
        scheduler_output.r9v_ced_approx = False
        if len(num_scheduled_tokens) == 1:
            ((req_id, count),) = num_scheduled_tokens.items()
            plan = self.requests[req_id].r9v_ced
            if plan is not None and self.requests[req_id].num_computed_tokens + count <= plan.tail_start:
                scheduler_output.r9v_ced_approx = True
                scheduler_output.r9v_ced_split = plan.split
                scheduler_output.r9v_ced_fill = plan.fill
        for request in scheduled_new_reqs:  # includes resumed ones: they prefill again
            if request.r9v_ced is not None:
                plan = request.r9v_ced
                logger.info("CED request %s: %d prompt tokens approximated from layer %d, exact tail %d of %d",
                            request.request_id, max(plan.tail_start - request.num_computed_tokens, 0),
                            plan.split, request.num_prompt_tokens - plan.tail_start, request.num_prompt_tokens)
    ...
    def _preempt_request(
        ...
        computed_before_reset = request.num_computed_tokens
        ...
        from vllm.v1.worker.r9v_diagnostics import record_preemption

        record_preemption(self, request, computed_before_reset)
        if self.log_stats:
    ...
    def add_request(self, request: Request) -> None:
        ...
        if existing is not None:
            ...
            request.r9v_ced = self._ced_plan(request)
            if request.r9v_ced is not None:
                # Own prefix-cache namespace: the salt enters the first block's hash, and every
                # later block (KV and GDN state alike) chains from it.
                request.cache_salt = f"{request.cache_salt or ''}|{request.r9v_ced.salt}"
