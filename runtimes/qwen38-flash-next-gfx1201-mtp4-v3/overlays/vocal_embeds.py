...
from .. import ops
from .iq4nl import dequantize_iq4_nl
...
_DEQUANT_ROWS = {"torch": 0, "gguf": 0}


def _torch_iq4_nl(rows: int) -> bool:
    """Same-server A/B only: the file R9V_PLE_DEQUANT_CONTROL names may hold "gguf".

    With the control set, the running row count per path is kept in <control>.rows so a
    driver can prove which path served each request."""
    control = os.environ.get("R9V_PLE_DEQUANT_CONTROL")
    if not control:
        return True
    try:
        with open(control) as f:
            path = "gguf" if f.read().strip() == "gguf" else "torch"
    except FileNotFoundError:
        path = "torch"
    _DEQUANT_ROWS[path] += rows
    with open(control + ".rows", "w") as f:
        f.write(f"torch={_DEQUANT_ROWS['torch']} gguf={_DEQUANT_ROWS['gguf']}")
    return path == "torch"
...
def _apply_gguf_embedding(
    ...
    if qweight_type in DEQUANT_TYPES:
        ...
        if qweight.device.type == "cpu":
            ...
            if weight_type == WeightType.IQ4_NL and _torch_iq4_nl(quant.shape[0]):
                # The n-gram table: multi-threaded torch, bit-identical to gguf-py.
                dequant = dequantize_iq4_nl(quant)
            else:
