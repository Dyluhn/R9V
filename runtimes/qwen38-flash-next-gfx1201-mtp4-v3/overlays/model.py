...
# The vision encoder's weights and the CED projector share one VRAM region per GPU.
...
def _shared_vram_setup(device: torch.device) -> _SharedVram:
    ...
    with torch.inference_mode(False):
        ...
        sets = {}
        for split in _CED_FILES:
            projector = _ced_load(_CED_FILES[split], torch.device("cpu"))
            _SHARED.setdefault("names", {})[split] = list(projector)
            sets[f"ced:{split}"] = list(projector.values())
    ...
    for split in _CED_FILES:
        logger.info("CED projector split=%d %s: %.3f GiB in pinned host RAM, shares VRAM with the vision encoder: %s",
                    split, _CED_PRECISION, shared.bytes[f"ced:{split}"] / 2**30, _CED_FILES[split])
...
def _ced_projector(split: int) -> dict[str, torch.Tensor]:
    """The split's projector maps (_ced_load's names) in the shared region, made resident."""
    views = _shared_vram().acquire(f"ced:{split}")
    return dict(zip(_SHARED["names"][split], views))
