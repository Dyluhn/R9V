...
# {"splits": [..], "stride": N, "subdir": "name", "layers": [..]} for the next forwards
# ("layers": earlier block inputs to record too, e.g. the full-attention layers 3/7/11/15 a
# multi-source projector reads); empty = the env defaults below. Projection targets are
# computed offline from the GGUF weights, so TP sharding never enters the capture.
...
def _kva_begin(num_tokens: int) -> None:
    ...
    _KVA.update(on=True, splits=set(splits), first=splits[0], layers=set(config.get("layers", [])),
                stride=int(config.get("stride", _KVA_STRIDE)), subdir=config.get("subdir", ""))


def _kva_block_input_wanted(layer_idx: int) -> bool:
    return _KVA["on"] and (layer_idx >= _KVA["first"] or layer_idx in _KVA["layers"])
...
# A multi-source projector (metadata "sources": capture keys, e.g. boundary_16 plus
# block_input_3/7/11/15) reads its features concatenated in that order: the boundary state and
# block inputs of layers before S, which the approximate chunk keeps as it computes them
# (_CED_KEEP, filled only in the uncompiled forward). Research: an R9V_CED_PROJECTOR entry
...
def _ced_sources_of(path: str, split: int) -> list[str]:
    """The capture keys a projector's features are made of, in order (metadata "sources";
    absent = the boundary state alone). Only states computed before the split are allowed."""
    from safetensors import safe_open

    with safe_open(path, "pt") as f:
        meta = f.metadata() or {}
    sources = json.loads(meta["sources"]) if "sources" in meta else [f"boundary_{split}"]
    for key in sources:
        if key != f"boundary_{split}" and not (key.startswith("block_input_") and int(key.rsplit("_", 1)[1]) < split):
            raise ValueError(f"CED projector {path}: source {key!r} is not boundary_{split} or a block input before layer {split}")
    return sources


def _ced_entries(entries: list[str]) -> dict[int, tuple[str, int]]:
    """{projector id: (path, split)}; an entry is "path" (id = its split) or "ID=path"."""
    out = {}
    for entry in entries:
        key, _, path = entry.rpartition("=")
        split = _ced_split_of(path)
        pid = int(key) if key else split
        if pid in out:
            raise ValueError(f"R9V_CED_PROJECTOR lists two projectors with id {pid} (the split unless ID=path): {entries}")
        out[pid] = (path, split)
    return out


_CED_ENTRIES = _ced_entries(_CED_PROJECTOR_PATHS)
_CED_FILES = {pid: path for pid, (path, _) in _CED_ENTRIES.items()}
_CED_SPLITS = {pid: split for pid, (_, split) in _CED_ENTRIES.items()}
_CED_SOURCES = {pid: _ced_sources_of(path, split) for pid, (path, split) in _CED_ENTRIES.items()}
_CED_KEEP: set[int] = set()  # block inputs the current approximate chunk's projector reads
_ced_kept: dict[int, torch.Tensor] = {}
...
def _ced_active_split() -> int | None:
    """The projector id (its split unless ID=path) this step approximates with, or None for an
    exact step."""
    if not _CED["approx"]:
        return None
    split = _CED["split"]
...
def _ced_record(num_tokens: int, started: float, spans: list, split: int, pid: int) -> None:
    ...
    chunk = {"t": time.time(), "request": _CED.get("request"), "tokens": num_tokens,
             "split": split, "projector": pid, "fill": _CED["fill"],
...
def _ced_load(path: str, device: torch.device) -> dict[str, torch.Tensor]:
    ...
    the CPU, so the bf16 copy never occupies VRAM. A file stored as int8 already (the int8 map
    plus "scale.<name>" and "bias.<name>", as kva/quantize_projector.py writes them with this
    same quantization) loads as stored."""
    from safetensors import safe_open
    from safetensors.torch import load_file

    with safe_open(path, "pt", device="cpu") as f:
        stored_int8 = any(name.startswith("scale.") for name in f.keys())
    if _CED_PRECISION == "bf16" or stored_int8:
...
class Qwen4ExpDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if _kva_block_input_wanted(self.layer_idx):
            _kva_keep(f"block_input_{self.layer_idx}", block_input)
        if _CED_KEEP and self.layer_idx in _CED_KEEP:  # approximate chunk, multi-source projector
            _ced_kept[self.layer_idx] = block_input.clone()  # the mix may reuse its output buffer
...
class Qwen4ExpModel(nn.Module):
    ...
    def forward(
        ...
        injection = None
        last_layer = None
        ced_id = _ced_active_split()
        ced_split = None if ced_id is None else _CED_SPLITS[ced_id]
        if ced_id is not None:  # the earlier block inputs a multi-source projector reads
            _CED_KEEP.update(int(k.rsplit("_", 1)[1]) for k in _CED_SOURCES[ced_id] if k.startswith("block_input_"))
        for layer_idx, layer in islice(
            enumerate(self.layers), self.start_layer, self.end_layer
        ):
            last_layer = layer
            if layer_idx == ced_split:
                return self._ced_approximate_rest(hidden_states, block_output, injection, positions, ced_split, ced_id)
    ...
    def _ced_approximate_rest(self, hidden_states, block_output, injection, positions, split, pid):
        """Fill layers split..end caches from projector pid's predictions (see module note)."""
        ...
        sources = _CED_SOURCES[pid]
        if sources != [f"boundary_{split}"]:  # multi-source: features in the file's declared order
            boundary = torch.cat([boundary if k == f"boundary_{split}" else _ced_kept[int(k.rsplit("_", 1)[1])].to(torch.bfloat16)
                                  for k in sources], 1)
        _CED_KEEP.clear()
        _ced_kept.clear()
        ...
        if timed:
            _ced_record(multi_hidden.shape[0], started, spans, split, pid)
