...
| `R9V_CED_PROJECTOR_REL` | `ced/ced-projector-split16.safetensors` | Projector for `on`, inside the model directory, read at `/models/…`: bf16, 1.76 GiB per GPU, in a VRAM region shared with the vision encoder's weights (0.42 GiB per GPU) since v0.4.4; both stay in pinned host RAM and take turns there. |
| `R9V_CED_QUALITY_PROJECTOR_REL` | `ced/ced-projector-split16-msfa-int8.safetensors` | Projector for `quality`: split 16 plus the block inputs of layers 3/7/11/15, stored as int8 (1.79 GiB per GPU), in a VRAM region shared with the vision encoder's weights (0.42 GiB per GPU); both stay in pinned host RAM and take turns there. Optional in the model package; setup downloads it only with `--ced quality`. |
...
| `R9V_MIN_HOST_AVAILABLE_BYTES` | `60424720384` | Available RAM before launch, 56.3 GiB: the mutable cache's host expert copy (40.3 GiB: every expert for rank 0, the 112 per layer rank 1 does not pin) plus the 16 GiB PLE reserve. The GPU test's peak draw was 49 GiB, so about 7 GiB stays spare. Was 71.4 GiB before v0.4.1. With `--ced on` or `quality` the doctor adds the pinned host copies of the projector (one per TP rank) and of the vision encoder's weights (the mmproj file's size): 4.50 GiB, so start needs 60.8 GiB (`on` since v0.4.4, `quality` since v0.4.3). |
