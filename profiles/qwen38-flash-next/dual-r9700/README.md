...
| `R9V_MIN_HOST_AVAILABLE_BYTES` | `60424720384` | Available RAM before launch, 56.3 GiB: the mutable cache's host expert copy (40.3 GiB: every expert for rank 0, the 112 per layer rank 1 does not pin) plus the 16 GiB PLE reserve. The GPU test's peak draw was 49 GiB, so about 7 GiB stays spare. Was 71.4 GiB before v0.4.1. With `--ced quality` the doctor adds the pinned host copies of the projector (one per TP rank) and of the vision encoder's weights (the mmproj file's size): 4.50 GiB, so start needs 60.8 GiB. |
