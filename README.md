...
**New in v0.4.2:** `qwen38-mtp4-uncensored` has an opt-in CED quality mode, `--ced quality`, that gives up less long-context quality for less prefill speedup. It is **not recommended**: on the reference host it fails its first-start VRAM check (below). Right after a first start, `./r9v doctor --runtime` no longer fails its KV-pressure check because of qualification's own long prompt. See the [changelog](CHANGELOG.md).
...
| `qwen38-mtp4-uncensored` | [Uncensored IQ4_XS bundle](https://huggingface.co/Dyluhn/Qwen3.8-Flash-Next-Uncensored-R9V-IQ4_XS) (abliterated, **no refusals**) | Consolidated 1.3.0 runtime with host expert dedupe: MTP4, dual R9700, 128K context, CED on; needs 56.3 GiB free RAM at start | v0.4.1 clean install (fetch, setup, first start CED on/off, restart) passed on the reference host; v0.4.2 `--ced quality` failed its first-start VRAM check there (not recommended, below) |
...
**`--ced quality` is not recommended.** In the clean-install GPU test on the compiled server it ran every workload check, including the 130,941-token prompt, but its peak VRAM is about 0.75 GiB higher on GPU 0 and 0.35 GiB higher on GPU 1 than `on`. With desktop apps holding 1.24 GiB of GPU 0, GPU 0 dropped to 0.90 GiB free (target 1.5 GiB), so first-start qualification failed and `start` exits with an error; `on` passed in the same session (1.65 / 2.12 GiB free). It can pass only if other programs use less than about 0.6 GiB on GPU 0. Where it ran, it behaved as designed: prefill 1.49× at 12.9K tokens and 1.64× at 32K (`on`: 1.54× and 1.74× in the same session), unchanged decode, exact requests bitwise identical after a CED request, no recompiles. See the [changelog](CHANGELOG.md). The profile uses a fixed expert placement, so it does not accept `--headroom`.
