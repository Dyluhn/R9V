...
What: `BatchMeta.slot_map` is specified as "`[G, T]` (one flattened slot per new token per group)" with `slot(s, p) = (block_table[g][s][p / 32], p % 32)`, which does not say whether the flattened value carries the pool-global block id or the within-table position; and `Sink(n) + Window(w)` retention keeps two ranges (pinned sink plus sliding window) while `BatchMeta.window_start` carries a single `[G, S]` value.
Why it blocks or misleads: Two managers that both satisfy the text can emit different `slot_map`/`window_start` bytes, breaking the Spec 3 §5 promise that identical request histories produce identical `BatchMeta` for doctor-bundle diffs; the scheduler and attention kernel cannot agree on slot decoding or the sink range without a stated convention.
Option taken: `slot = pool_global_block_id * 32 + lane` (so the device derives the arena offset directly); `window_start` reports the window start (`ctx_len - w`) with the sink range implicitly `[0, ceil(n/32)*32)` pinned from position 0.
Proposed resolution: State the flattening as `block_id * 32 + lane` with pool-global ids, and state that `window_start` is the window start while the sink length derives from the group's `Retain`.
